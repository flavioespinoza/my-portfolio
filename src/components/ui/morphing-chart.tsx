...
function getTickFormat(groupBy: 'day' | 'month' | 'week' | 'year'): string {
  switch (groupBy) {
    ...
    case 'week': return '[W]W'
    case 'year': return 'YYYY'
  }
}

function MorphingChart({ projectFilter = 'all', search = '', groupBy, getFiltered }: MorphingChartProps) {
  ...
  useEffect(() => {
    ...
    xAxisGroup.selectAll('.tick')
      .style('display', (d, i) => i % Math.ceil(timeGroups.length / tickCount) === 0 ? 'block' : 'none')
    ...
    if (showingPie) {
      const arcs = pieGroup
        ...
        .join('path')
        .attr('fill', (d: d3.PieArcDatum<{ project: string; total: number }>) => color(d.data.project))
        .attr('d', arc)
        .on('mouseover', function (event: MouseEvent, d: d3.PieArcDatum<{ project: string; total: number }>) {
